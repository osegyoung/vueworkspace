...
router.get('/books/:no', async (req,res)=>{
    ...
    let bookInfo = await bookService.findByBookNo(bookNo)
                                    .catch(err => console.log(err));
    res.send(bookInfo);
})
...
// 수정    : 자원(데이터) -> books / 수정 -> PUT
router.put('/books/:no', async(req, res)=>{
    let bookNo = req.params.no;
    let bookInfo = req.body;

    let result = await bookService.modifyBookInfo(bookNo, bookInfo)
                                    .catch(err => console.log(err));
    res.send(result);
});
  
// 삭제    : 자원(데이터) -> books / 삭제 -> DELETE
router.delete('/books/:no', async (req, res)=>{
    let bookNo = req.params.no;
    let resInfo = await bookService.removeBookInfo(bookNo)
                                    .catch(err => console.log(err));
    res.send(resInfo);
});
